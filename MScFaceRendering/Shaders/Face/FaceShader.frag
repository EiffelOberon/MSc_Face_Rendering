...
float shadowPCF(vec3 screenSpaceCoords, float testPointZ, float samples){	
	...
	return (visibilityFactor/(samples*samples)) - 0.3;
}
...
fragmentColor.rgb = visibility * ((1.0-rho_s)*diffuseLightingContribution.rgb*diffuseColor + rho_s*specularLightingContribution.rgb);
fragmentColor.a = 1.0;
